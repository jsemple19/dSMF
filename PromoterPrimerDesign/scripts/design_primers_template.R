...
######################### define ranges differently - by using GR and BSgenome ################################3
	#########################
	#Define the ranges
	##########################
#	RESTsites=readRDS("/work/gbioinfo/stadler/groups/schuebeler/schoeler_anne/REST/motif_v3/RESTsites_mm9Mappable_scoreCut1500_enr200_gr.rds")
#  	genome(RESTsites)='ce11'
#	target_regions=RESTsites[order(RESTsites$enr200.ES,decreasing=T)][1:100]
#	target_regions2=RESTsites[order(RESTsites$enr200.ES,decreasing=T)][200:300]

#   DCCgenes<-data.frame(WBids=read.table("../dccGenes_Rahul/DCCgenes.txt")[,1],publicIDs_WS220=read.table("../dccGenes_Rahul/DCCgenes_publicIDs_WS220.txt")[,1],
#                          publicIDs_WS245=read.table("../dccGenes_Rahul/DCCgenes_publicIDs_WS245.txt")[,1])
#   if (sum(DCCgenes[,2]!=DCCgenes[,3])) {
#     print("gene names have changed")
#   }
...
#   #gnId<-gsub("Gene:","",gnId)
#   #chooseCols<-c("CDSNAME","CDSCHROM","CDSSTRAND","CDSTART","CDSEND")
#   targets<-promRegSel[as.character(DCCgenes[,1]),]
#   seqlevels(targets)<-seqnames(Celegans)#sort(seqnames(Celegans))
#   target_regions<-targets
# #  target_regions2<-targets
  
  ##########
  chosenTSS<-readRDS("chosenTSS.Rds")
  TSS<-readRDS("../../TSS/scripts/confirmedTSS_Chen&Kreus_WS235.Rds")
  names(TSS)<-mcols(TSS)$WBGeneID
  
  chosenGR<-lapply(chosen,function(x){
    TSS[x]
  })
  ...
  #########################
	...
	###########################
		...
		maxD=100 #max number of regions considered
	#########################
	#Design primers
	###########################
      ...
      target=igr#ectg.tss[ectg.tss.i]
			...
			refGR=GRanges(as.character(seqnames(snps))[nai],
				IRanges(
				ifelse(strand[nai]=='+',stP[nai]+dfr$FwPos[nai]-1,
				enP[nai]-dfr$FwPos[nai]-dfr$fragLen[nai]-1
				),
				ifelse(strand[nai]=='+',stP[nai]+dfr$FwPos[nai]+dfr$fragLen[nai]-1 ,enP[nai]-dfr$FwPos[nai]-1) 
	...
	names(reg)=sl(reg)
	cbind(as.data.frame(reg)[,1:3]	,rep('.',length(reg)),rep('.',length(reg)),as.data.frame(reg)[,5])
	
	
	#export the list containing bot  primers and reference GR
	saveRDS(design ,'tmp_design_test.rds')
	...
	write.table(primer.df[!(duplicated(primer.df$Fwseq)&duplicated(primer.df$Rvseq)),],
	            paste0('./DM_DE_promoter_test_primers_',genomeVer,'.tab'),sep='\t',quote=F)

	#export the bed file for UCSC visualization
	write.table(cbind(as.data.frame(reg)[,1:3]	,rep('',length(reg)),rep('',length(reg)),as.data.frame(reg)[,5]),
	            paste0('./test_primers_bed_',genomeVer,'.bed'),sep='\t',quote=F,row.names=F,col.names=F)

	# #export the target gene list
	# export(target,paste0('./DCCgenes_primerTargets_',genomeVer,'.bed'))
	# 
	# #export the target gene list for which amplicons were designed		
	# hits<-findOverlaps(reg,targets)
	# targets_withAmplicons<-targets[queryHits(hits)]	
	# export(targets_withAmplicons,paste0('./DCCgenes_targetsWithAmplicons_',genomeVer,'.bed'))
	# paste0(seqnames(targets_withAmplicons),":",start(targets_withAmplicons)-500,"-",
	#        end(targets_withAmplicons)+500)
	# 