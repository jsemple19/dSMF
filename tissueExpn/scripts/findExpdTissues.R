...
dataFiles<-list.files("../Spencer2011/",pattern="*.txt",recursive=TRUE)
#take tissue data that has NOT been filtered by whole animal reference level
dataFiles<-dataFiles[grep("expressedGenes_unfiltered",dataFiles)]
#system("ls ")
#list.dirs("../Spencer2011")
# get list of genes that appear in these lists
geneList=list()
for (t in seq_along(dataFiles)){
...
#genuinely widely expressed genes shows that housekeeping genes are not properly included
